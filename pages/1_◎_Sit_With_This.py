...
with st.sidebar:
    ...
    st.markdown(f"""
    <div style="font-size: 0.78rem; color: #000; line-height: 1.7;">
        <strong style="color: #000;">{len(TEACHINGS)}</strong> teachings<br>
        <strong style="color: #000;">{len(get_philosophers())}</strong> philosophers<br>
        <strong style="color: #000;">{len(get_traditions())}</strong> traditions<br>
        <strong style="color: #000;">{len(THEMES) - 1}</strong> themes
    </div>
    """, unsafe_allow_html=True)
