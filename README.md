...
- **pyproject.toml** - Modern Python project configuration (used by `uv`)
- **requirements.txt** - Traditional pip dependencies (for deployment)
- **uv.lock** - Ensures reproducible installs across environments
- Legacy files (main.py, SitwithIt.py, observer_app.py) are kept for backward compatibility

### Why `uv`?

[uv](https://github.com/astral-sh/uv) is a modern, blazing-fast Python package installer and resolver:
